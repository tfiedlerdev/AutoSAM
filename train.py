...
from dataset.retina import get_retina_dataset
...
def postprocess_masks(masks_dict):
    masks = (
        torch.zeros((len(masks_dict), *masks_dict[0]["low_res_logits"].squeeze().shape))
        .unsqueeze(dim=1)
        .cuda()
    )
...
def train_single_epoch(ds, model, sam, optimizer, transform, epoch):
    ...
    for ix, (imgs, gts, original_sz, img_sz) in enumerate(pbar):
        ...
        orig_imgs_small = F.interpolate(
            orig_imgs, (Idim, Idim), mode="bilinear", align_corners=True
        )
        ...
        pbar.set_description(
            "(train | {}) epoch {epoch} ::"
            " loss {loss:.4f}".format("Medical", epoch=epoch, loss=np.mean(loss_list))
        )
...
def inference_ds(ds, model, sam, transform, epoch, args):
    ...
    for imgs, gts, original_sz, img_sz in pbar:
        ...
        masks = sam.postprocess_masks(
            masks, input_size=input_size, original_size=original_size
        )
        gts = sam.postprocess_masks(
            gts.unsqueeze(dim=0), input_size=input_size, original_size=original_size
        )
...
def main(args=None, sam_args=None):
    ...
    elif args["task"] == "retina":
        assert args["target"] is not None
        trainset, testset = get_retina_dataset(
            args, sam_trans=transform, target=args["target"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-lr", "--learning_rate", default=0.0003, help="learning_rate", required=False
    )
    parser.add_argument(
        "-bs", "--Batch_size", default=3, help="batch_size", required=False
    )
    parser.add_argument(
        "-epoches", "--epoches", default=5000, help="number of epoches", required=False
    )
    parser.add_argument(
        "-nW", "--nW", default=0, help="evaluation iteration", required=False
    )
    parser.add_argument(
        "-nW_eval", "--nW_eval", default=0, help="evaluation iteration", required=False
    )
    parser.add_argument(
        "-WD", "--WD", default=1e-4, help="evaluation iteration", required=False
    )
    parser.add_argument(
        "-task", "--task", default="glas", help="evaluation iteration", required=False
    )
    parser.add_argument(
        "-depth_wise", "--depth_wise", default=False, help="image size", required=False
    )
    parser.add_argument(
        "-order", "--order", default=85, help="image size", required=False
    )
    parser.add_argument(
        "-Idim", "--Idim", default=512, help="image size", required=False
    )
    parser.add_argument(
        "-rotate", "--rotate", default=22, help="image size", required=False
    )
    parser.add_argument(
        "-scale1", "--scale1", default=0.75, help="image size", required=False
    )
    parser.add_argument(
        "-scale2", "--scale2", default=1.25, help="image size", required=False
    )
    parser.add_argument(
        "-t",
        "--target",
        default=None,
        help="Target for retina dataset",
        required=False,
        choices=["disc", "cup"],
    )
    ...
    base_out = (
        os.path.join("results", args["target"])
        if args["target"] is not None
        else "results"
    )
    os.makedirs(base_out, exist_ok=True)
    folder = open_folder(base_out)
    out_dir = os.path.join(base_out, "gpu" + folder)
    os.makedirs(out_dir, exist_ok=True)
    args["folder"] = folder
    args["path"] = os.path.join(out_dir, "net_last.pth")
    args["path_best"] = os.path.join(out_dir, "net_best.pth")
    args["vis_folder"] = os.path.join(out_dir, "vis")
